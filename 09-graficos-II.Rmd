...
La variable `temperatura` es una tabla que tiene dato de temperatura del aire para cada tiempo, nivel (altura), latitud y longitud. Como este archivo tiene información sólo para un tiempo y el código filtró sólo datos de un nivel, entonces la variació es sólo en latitud y longitud. ¿Cómo se grafica?
...
Puntos redondos y separdos no es la forma más informativa de ver estos datos. Una grilla regular en relaidad define como una "imagen" donde cada punto es un pixel. Esto se llama un "raster" y con ggplot se puede graficar con `geom_raster()`.
...
Bien, hasta ahí tenés un gráfico con los datos, pero para entenderlos en su contexto espacial como mínimo hace falta un mapa. Para tener un mapa se neceistan datos de la localización de las costas o las fronteras. Existen varias fuentes de estos datos, pero una muy buena es [Natural Earth](https://www.naturalearthdata.com/). El paquete {rnaturalearth} provee una interfaz amigable para usar estos datos directamente en {ggplot2}. 

Primero hay que cargar el mapa que querramos. Por ejemplo, para graficar el mapa de Argentina y sus países limítrofes cargamos los datos con `ne_countries()`:
...
Entonces ahora, para graficar la temperatura y el mapa encima, armarmos el gráfico de raster igual que antes y le sumamos el mapa:
...
Existen dos convenciones básicas para medir la longitud, medida entre 0ª y 360º o entre -180ª y +180ª. Los datos de temperatura usan la primera y los del mapa, la segunda. Para convertir entre una y otra convención, podés usar la función `ConvertLongitude()` de {metR}:
...
Las líneas dibujadas por `geom_contour()` son líneas de nivel que unen puntos de valor constante. Sin embargo, esas líneas azules no dan ninguna indicación del valor de la variable por lo que es conveniente mapear el color de las líneasa ese valor constante.
...
¿De dónde salió ese `stat(level)`? Algunso geoms que realizan transofrmaciones estadísticas computan variables nuevas que luego se pueden mapear a parámetros geométricos. "level" es una de esas variables computadas de `geom_contour`. Para decirle a ggplot que al level al que te referís es a la variable computada, se rodea a la variable con la función `stat()`.
...
Si vas a la ayuda de `kriging()`, vas a ver que requerie un vector de coordenadas x, un vector de coordenadas y y un vector de valores observados. Estos son las columnas `lon`, `lat` y `tmax_media` de la tabla `observaciones`. Entonces el código sería:
...
¿Qué es lo que devolvió la función `kriging()`? Podés mirar la doumentación de la función (en la sección "Value") o usando `glimpse(observaciones_regular)`.
...
```
La mayoría de los elementos brindan información detallad sobre los parámetros usados y computados para relaizar la interpolación. El elemento útil es el que se llama `map`, que tiene los datos interpolados. ¿Qué es lo que tiene `map`?
...
La interpolación de este gráfico queda fea en los bordes porque tiene poca resolución. ¿Qué hay que cambiar en la llamda a `kriging()` para incrementarla? Fijate en la documentación de la función. 
