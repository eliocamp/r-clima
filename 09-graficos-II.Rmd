...
::: {.alert .alert-success} 
**Nota** 
Para que `geom_contour_fill()` funcione bien en este caso, hay que instalar la versión de desarrollo de `metR`que se puede instalar con el comando `remotes::install_gihub("eliocamp/metR@dev")`. 
También es necesaria esa versión para que funcione `geom_text_contour()` como en el ejemplo que sigue.
:::
